...
async function updateOffersFromSheet() {
    const CSV_URL = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vQ9hPn5l-8ASjL1236ah9LJf4VBi8QSw531JhWp7-7PMSixmI9xMJmqHQ_SQwYwBODAnV224CEhrdmv/pub?output=csv&gid=1204848822';
    ...
    try {
        ...
        console.error('[LAG Volvo] Error fetching sheet:', err);
        return;
    }
    ...
    document.querySelectorAll('.car-offer').forEach(section => {
        ...
        // Collect for anchor nav — strip year + make from Model Title (e.g. "2026 Volvo XC40" → "XC40")
        const sectionId = section.id;
        const rawTitle = (data['Model Title']?.value || '').trim();
        const navLabel = rawTitle.replace(/^\d{4}\s+\S+\s+/i, '').trim();
...
function waitForOffersToLoad(retries = 20) {
    ...
    else console.warn('[LAG Volvo] .car-offer not found after retries — script aborted.');
}
waitForOffersToLoad();
